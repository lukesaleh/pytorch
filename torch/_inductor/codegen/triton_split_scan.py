...
class TritonSplitScanKernel(TritonKernel):
    ...
    def scan(self, dtypes, combine_fn, values):
        ...
        value = cse_compute(f"{value}.to({compute_type})", dtype=dtype,)
        value = cse_compute(
            f"tl.broadcast_to({value}, {self.dense_size_str()})", dtype=dtype,
        )
        ...
        block_sum = cse_compute(
            f"tl.reduce({value}, {dim}, {combine_helper_fn})", dtype=dtype,)
